...
class overdrive : public dsp {
  ...
  public:
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		float 	fSlow1 = (0.0001f * faustpower<2>(fSlow0));
		float 	fSlow2 = (0.01f * fSlow0);
		float 	fSlow3 = fslider1;
		float 	fSlow4 = (fSlow3 - 1);
		float 	fSlow5 = (0.0010000000000000009f * powf(10,(0.05f * (0 - (0.5f * fSlow3)))));
		float 	fSlow6 = (1 - fSlow2);
		FAUSTFLOAT* input0 = input[0];
		FAUSTFLOAT* output0 = output[0];
		for (int i=0; i<count; i++) {
			float fTemp0 = (float)input0[i];
			float fTemp1 = fabsf((fSlow2 * fTemp0));
...
#ifndef PLUGIN_URI
#define PLUGIN_URI URI_PREFIX "/overdrive"
#endif
...
#endif
  if (!plugin_name) plugin_name = "overdrive";
